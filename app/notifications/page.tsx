  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white font-['Space_Grotesk']">Notifications</h1>
          <p className="text-neutral-400 mt-1">{unread} unread notification{unread !== 1 ? "s" : ""}</p>
        </div>
        {unread > 0 && (
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
            onClick={markAll}
            className="flex items-center gap-2 px-4 py-2 rounded-xl glass border border-white/10 text-neutral-400 hover:text-white text-sm transition-all">
      ...
      <div className="space-y-2">
        {loading ? (
          Array.from({ length: 5 }).map((_, i) => <div key={i} className="h-16 bg-white/3 rounded-xl animate-pulse" />)
