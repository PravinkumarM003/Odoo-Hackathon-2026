...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      if (isHR) {
        const [att, notifs, anns, birds, hr] = await Promise.all([
          attendanceApi.getToday(),
          notificationsApi.getMy(),
          fetch("/api/announcements").then(r => r.ok ? r.json() : []),
          fetch("/api/dashboard/leaderboard").then(r => r.ok ? r.json() : []),
          hrApi.getActionCenter()
        ]);
        setTodayAttendance(att as any);
        setNotifications(notifs as any);
        setAnnouncements(anns as any);
        setEarlyBirds(birds as any);
        setHrData(hr as any);
      } else {
        const [att, notifs, anns, birds, bal, pay] = await Promise.all([
          attendanceApi.getToday(),
          notificationsApi.getMy(),
          fetch("/api/announcements").then(r => r.ok ? r.json() : []),
          fetch("/api/dashboard/leaderboard").then(r => r.ok ? r.json() : []),
        ...
        setAnnouncements(anns as any);
        setEarlyBirds(birds as any);
        setLeaveBalance(bal as any);
        setPayroll(pay as any);
      }
    } catch {
